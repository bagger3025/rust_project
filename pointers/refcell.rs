...
impl<T> RefCell<T> {
    ...
    pub fn borrow(&self) -> Option<Ref<'_, T>> {
        match self.state.get() {
            RefState::Unshared => {
                ...
                // since state would be Exclusive.
    ...
    pub fn borrow_mut(&self) -> Option<RefMut<'_, T>> {
        if let RefState::Unshared = self.state.get() {
            ...
            // SAFETY: no other references have been given out
            // since state would be Shared or Exclusive.
...
impl<T> Deref for Ref<'_, T> {
    ...
    fn deref(&self) -> &Self::Target {
        ...
        // so no exclusive references are given out.
...
impl<T> DerefMut for RefMut<'_, T> {
    fn deref_mut(&mut self) -> &mut Self::Target {
        // SAFETY:
        // a RefMut is created if no other referenceshave been given out.
        // once it is given out, state is set to Exclusive,
        // so no future references are given out.
        // so we have an exclusive lease on the inner value,
